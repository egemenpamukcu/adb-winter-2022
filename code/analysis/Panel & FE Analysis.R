...
install.packages("C:\\Users\\linst\\OneDrive\\Desktop\\collapse_1.7.6.tar.gz")
library(AER)
library(plm)
...
fsi_climate.model <- plm(state_legit ~ air_quality + climate_change + hdi_value + gdp, data = df_imputed, index = c("iso", "year"), model = "within", effect = "twoways")
fsi_migration.model <- plm(state_legit ~ migrant_stock + refugee_stock + conflict_displacement + disaster_displacement + gdp, data = df, index = c("iso", "year"), model = "within", effect = "twoways")
fsi_governance.model <- plm(state_legit ~ rule_of_law + gov_effectiveness + corruption_control + gdp, data = df, index = c("iso", "year"), model = "within", effect = "twoways")
...
cpa_climate.model <- plm(cpa_d_avg ~ air_quality + climate_change + hdi_value + gdp, data = df, index = c("iso", "year"), model = "within", effect = "twoways")
cpa_migration.model <- plm(cpa_d_avg ~ migrant_stock + refugee_stock + conflict_displacement + disaster_displacement + gdp, data = df, index = c("iso", "year"), model = "within", effect = "twoways")
cpa_governance.model <- plm(cpa_d_avg ~ rule_of_law + gov_effectiveness + corruption_control + gdp, data = df, index = c("iso", "year"), model = "within", effect = "twoways")
...
if (SAVE.RESULTS) {
  stargazer(cpa_climate.model, cpa_migration.model, cpa_governance.model, type='html', out=paste(output.tab.dir, '/state_legit_reg_fe.html', sep=''))
}
summary(model)